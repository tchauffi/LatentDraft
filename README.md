...
- **Linux** — [bubblewrap](https://github.com/containers/bubblewrap) when the kernel allows unprivileged user namespaces: no network, no filesystem beyond the build directory, its own PID/IPC/UTS namespaces, no `$HOME`, no repo source. Where user namespaces are blocked (Ubuntu ≥23.10 sets `kernel.apparmor_restrict_unprivileged_userns=1` by default), the **Landlock** LSM is used instead — the interpreter restricts *itself* before running your snippet, with reads allowlisted to system paths plus the build directory, and writes confined to the build directory. Landlock also denies TCP, though only from ABI 4 (kernel 6.7) up and never UDP, so the network is closed by the seccomp filter below rather than by Landlock. Both are inherited by anything the snippet spawns and cannot be undone.
...
- **Everywhere** — hard rlimits the snippet cannot raise (address space, max file size, CPU, no core dumps), a 30 s wall clock that kills the whole process group, and an environment **stripped of every API key** before the interpreter starts. Where the OS layer does not already unshare the network (so: Landlock, and the unsandboxed fallback), a **seccomp filter** refuses `AF_INET`/`AF_INET6` sockets outright — TCP and UDP alike, on every kernel — while `AF_UNIX` keeps working so `multiprocessing` still does. A socket patch inside the interpreter sits behind it, purely so the snippet gets "load data from files in this directory instead of URLs" rather than a bare `EACCES`.
